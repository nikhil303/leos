...
@DesignRoot("DocumentCardDesign.html")
public class DocumentCard extends VerticalLayout implements Card{
    ...
    protected Button proposalButton;
    ...
    @Autowired
    public DocumentCard(UserHelper userHelper, MessageHelper messageHelper, LanguageHelper langHelper, EventBus eventBus) {
    ...
    private void initializeStaticData() {
        ...
        proposalButton.setCaption(messageHelper.getMessage("card.button.caption.proposal"));
    }
        ...
        openButton.addClickListener(new Button.ClickListener() {
            ...
            public void buttonClick(Button.ClickEvent event) {
                eventBus.post(new SelectDocumentEvent((String) itemDataSource.getItemProperty(KEY).getValue(), 
                        (LeosCategory) itemDataSource.getItemProperty("documentType").getValue()));
            }
        });

        proposalButton.addClickListener(new Button.ClickListener() {
            @Override
            public void buttonClick(Button.ClickEvent event) {
                eventBus.post(new DisplayCollectionEvent((String) itemDataSource.getItemProperty(KEY).getValue(),
                        (LeosCategory) itemDataSource.getItemProperty("documentType").getValue()));
            }
        });
    ...
    public void setItemDataSource(Item item) {
        ...
        addStyleName("document");
        addStyleName((item.getItemProperty("documentType").getValue()).toString());
    }
